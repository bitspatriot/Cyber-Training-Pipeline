...
**Status:** Active · **Last updated:** 2026-09-12 · **Maintainer:** Justin
...
| DC (`WIN-SLBA0U0E53P`) | Win Server 2022, AD DS + DNS | VLAN20 | `10.10.20.10` |
| Data-Node | Rocky Linux, server | VLAN20 | `10.10.20.20` |
| Infra-Node | Debian, time source + bastion | VLAN20 | `10.10.20.30` |
| Win11 endpoint (`WIN11-ENDPOINT`) | domain-joined **admin workstation** (RSAT) | VLAN10 | DHCP `10.10.10.100–200` |
| Host workstation | physical admin box | Default Switch | `172.29.x` (DHCP) |

The domain is administered **from the Win11 endpoint** via RSAT (AD Users & Computers
`dsa.msc`, Group Policy Management `gpmc.msc`), not from the DC console — a GUI-less
practice is that a server is administered from a workstation, not at its own console.
...
## Active Directory, GPOs & the mapped share

**Domain:** `squadron.internal`, single DC `WIN-SLBA0U0E53P`.

**Objects** (created via PowerShell — objects and links have cmdlets):
- `OU=Workstations` — holds the Win11 computer object (moved out of the default `CN=Computers`, which can't have GPOs linked).
- `Operators` — global security group; scope for the drive map and the password policy.
- Share `\\WIN-SLBA0U0E53P\OperatorsData` (`C:\Shares\OperatorsData` on the DC): Operators = Change/Modify, Domain Admins = Full.

**GPOs:**
- **Drive map** (`GPP - Map OperatorsData Drive`, linked to `OU=Workstations`): a User-Configuration GPP drive map (M: → the share) filtered to Operators by item-level targeting, **with Computer-side loopback processing (Merge)**. Loopback is mandatory — the OU holds a computer, and without it the user-side map applies to nothing while reporting success. Authored in GPMC (GPP has no cmdlet); the click-path is documented in the task writeup.
- **Logon auditing** (`Audit - Logon Events`, linked domain-wide): Advanced Audit Policy → Logon/Logoff → Audit Logon = Success + Failure. Domain-wide because auditing is baseline hardening and is not retroactive — an event never enabled is never recorded.

**Password policy (PSO, not a GPO):** `PSO-Operators` — 14-char minimum, complexity enabled, scoped to the Operators group. Account password policy takes effect only at the domain level or via a Password Settings Object; an OU-linked GPO is silently ignored for it, and the PSO is the only mechanism that scopes to a subset rather than the whole domain. The Default Domain Policy is left untouched.

**Verified:** M: maps for Operators members at logon; 4624/4625 events record in the Security log; `Get-ADUserResultantPasswordPolicy` returns `PSO-Operators` for members and nothing (domain default) for non-members.
...
- **Objects and links via cmdlet; GPP via GUI.** OUs, groups, GPOs, links, and PSOs are created with PowerShell and the commands recorded; Group Policy Preferences (the drive map) have no cmdlet and are authored in GPMC with the clicks documented.
- **Loopback for user settings on a computer OU.** Any User-Configuration policy linked to a computer OU needs Computer-side loopback processing or it silently does nothing.
- **Password policy is a PSO, not a GPO.** Scoped to a group; the Default Domain Policy stays untouched.
- **Disable Hyper-V time integration on Windows guests** (DC and endpoint) so they take domain/NTP time, not the hypervisor clock (the VMIC provider outranks configuration).
- **Administer from the workstation.** The domain is managed from the RSAT-equipped Win11 endpoint, not the DC console.
